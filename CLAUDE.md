...
| 2026-07-01 | MCP best-practice pass: capabilities + check_spec tools, Resources + Prompts, annotations | Match patterns from MATLAB MCP / mcp-builder (resources, static check, capability detect). Verified 21 tools/3 resources/2 prompts over stdio. |
| 2026-07-01 | #7 Subsystem authoring: component.schematic -> nested Schematic + external Terminals (by port Index) | Encapsulates control/compensator into one block; removes flat-only limit. Verified: encapsulated-controller buck regulates Vo=15.00 on live PLECS. |
| 2026-07-01 | #8 Thermal readout: HeatPipe connections + verified thermal terminals/params; heat flow read end-to-end | Closes M5 gap for a purpose-built thermal model (50 W = 100/2 on live PLECS). Semiconductor loss-data authoring still future. |
| 2026-07-01 | #9 CI (GitHub Actions: ruff + offline pytest on 3.10-3.12) + packaging metadata; ruff-clean; fixed duplicate KB "Sum" | Public repo now has automated testing; lint hygiene. |
| 2026-07-02 | Big-project A: semiconductor loss + junction-temp authoring via `plecs_build_model(thermal=[...])` | Losses/Tj need the device INSIDE an active HeatSink frame (a wired thermal terminal errors on live PLECS). Generate a `SemiconductorLibrary` loss XML into `<model>_plecs/`, enclose the device in a HeatSink (Frame+HeatPort), route to a HeatFlowMeter->Rth->ambient network. Probe names are device-type-specific & case-sensitive (wrong string silently reads 0). Verified buck Tj 25->32.4 C, ~4.9 W. Also fixed serializer bug: only one top-level Output Terminal emitted -> multi-output models returned 1 signal. See docs/thermal-magnetic-notes.md. |
| 2026-07-02 | Big-project B: magnetic permeance-network authoring (winding + permeance, `kind:"Magnetic"`) | Magnetic domain is pure symbolic connectivity (no geometric constraint). Added MagneticInterface (winding; 1,2 electrical + 3,4 magnetic; param n), MagneticPermeance (P), MagneticResistance (R), ACVoltageSource to KB CORE; serializer needed no change ("Magnetic" kind passes through). Verified inductor L=n^2*P=0.0996 H and transformer V2/V1=n2/n1=0.5000 on live PLECS. Magnetic circuits use layout="manual" (auto-layout is power-focused). |
| 2026-07-02 | Big-project C: control-diagram auto-layout — signal blocks get a dedicated rail | Root cause of the "messy" schematics: control blocks not in a hardcoded set fell through to power-rail placement (PI/Sat/comparator/carrier landed on the top rail at y=95). Fix: classify a control/signal block by DOMAIN (no Wire/Magnetic/HeatPipe connection) and place all of them on a rail at y=260, left-to-right by signal-flow depth (longest-path layering), stacking same-depth blocks. Verified: closed-loop buck control chain now a clean Vref->Err->PI->Sat->Cmp row, no overlaps, still regulates Vo=14.998 V. |
